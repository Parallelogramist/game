  ...
  test('every run-scope target is reachable in one expedition and rewards are positive', () => {
    for (const quest of EXPEDITION_QUESTS) {
      for (const step of quest.steps) {
        ...
        if (step.trigger.kind === 'clearHazard') {
          // Simulated over 300 run salts at the live seed 20260727, a run that enters every
          // sector rolls a mean of 3.7 ambush nests (p10 2), so a run-scope hive hunt above
