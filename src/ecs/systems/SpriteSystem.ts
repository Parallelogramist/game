 ...
 *
 * Performs viewport culling: entities outside the view rect (plus margin)
 * are set invisible so the WebGL renderer skips them entirely.
 */
export function spriteSystem(world: IWorld, view: WorldRect): IWorld {
  ...
  for (let i = 0; i < entities.length; i++) {
    ...
    if (sprite) {
      ...
      const onScreen = entityX >= cull.minX && entityX <= cull.maxX && entityY >= cull.minY && entityY <= cull.maxY;
      if (onScreen) {
